import os
#from viz import visualize_dataset_traj_threebody
import torch
...
def threebody_loc(configs):
    ...
    with torch.no_grad():
        ...
        #T = SAMPLES[type]["T"]
        #T = round(T,abs(Decimal(str(dt)).as_tuple().exponent))
        TIMEPOINTS = configs["timepoints"]
        #print("T = {}\n dt = {}\n points = {}".format(T,dt,TIMEPOINTS))
        
        dataset, t = data_maker.dataset_onekind(type,NSAMPLES,TIMEPOINTS,phi_span)
        #visualize_dataset_traj_threebody(dataset.cpu())
    ...
    modelODE = ode_mlp(dim,512,dim,1,["tanh","relu",""]).to(device_util.DEVICE)
    modelMLP = mlp(dim,512,dim,2,["tanh","relu","relu",""]).to(device_util.DEVICE)
    modelGRU = GRU_ODEINT(dim,dim,512).to(device_util.DEVICE)
    modelRNN = RNN_ODEINT(dim,dim,512).to(device_util.DEVICE)
    modelGRE = GRU_ODEINT_EULER(dim,dim,512).to(device_util.DEVICE)
    modelRNE = RNN_ODEINT_EULER(dim,dim,512).to(device_util.DEVICE)
