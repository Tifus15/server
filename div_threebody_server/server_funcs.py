...
def dataset_train(config):
    ...
    for i in range(len(data)):
        ...
        xs,hs = make_snapshots(xdx,data[i][2],BATCHTIME) 
...
## DEFINE LOSS and OPTISdict={}
    if CUDA:
        ...
        GHNN = GNN_maker_HNN(g,4,LAYER_SIZE,E_SIZE,["tanh",""],type=LAYER,bias=BIAS,dropout = 0.65)
        print(GHNN)
        ## DEFINE BASELINE
        HNN_model = HNN(12,LAYER_SIZE,["tanh"," "],bias =BIAS)
        print(HNN_model)
        loss_container=torch.zeros(EPOCHS,24)
    GRUHNN = rollout_GNN_GRU(g,4,LAYER_SIZE,E_SIZE,["tanh"],bias=BIAS,type=LAYER,dropout = 0.65)
    print(GRUHNN)
