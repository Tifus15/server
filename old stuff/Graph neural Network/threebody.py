...
class ham_func_3body(nn.Module):
    def __init__(self,M,G):
        ...
        self.A = torch.zeros(12,12).double().to(device_util.DEVICE)
        self.A[0,6]=1/M[0]
        self.A[1,7]=1/M[0]
        self.A[2,8]=1/M[1]
        self.A[3,9]=1/M[1]
...
class threebody:
    ...
    def make_sample(self,key,points,alpha):
        ...
        F = ham_func_3body(init_dict["M"],1.0).to(device_util.DEVICE)
        if alpha != 0:
            q = (y0[:,0:2] + y0[:,2:4] + y0[:,4:6])/3
            #print("alpha found!")
            R = torch.Tensor([[math.cos(alpha),-math.sin(alpha)],
                              [math.sin(alpha),math.cos(alpha)]]).double()
            ...
            y0[:,10:12] @= R.T
            """
        x = odeint(F,y0.to(device_util.DEVICE),t,method="dopri5")
    ...
    def hamiltonian(self,dataset):
        ...
        print(dataset.shape)
        T = 0.5*(torch.norm(dataset[:,:,6:8],dim=-1)**2 + torch.norm(dataset[:,:,8:10],dim=-1)**2 + torch.norm(dataset[:,:,10:12],dim=-1)**2)
        r12 = torch.norm(dataset[:,:,0:2]-dataset[:,:,2:4],dim=-1)
        r13 = torch.norm(dataset[:,:,0:2]-dataset[:,:,4:6],dim=-1)
        r23 = torch.norm(dataset[:,:,2:4]-dataset[:,:,4:6],dim=-1)
        U = -(1/r12 + 1/r13 + 1/r23)
    ...
    def dataset_onekind(self,key,nsamples, points,phi_span=[0 ,2*math.pi]):
        ...
        for i in tqdm(range(nsamples)):
            x, t = self.make_sample(key,points,phi_span[0]+torch.rand(1,)*(phi_span[1]-phi_span[0]))
            dataset[:,i,:,:] = x
    ...
    def dataset_one(self,points,key,alpha=0):
        x,t = self.make_sample(key,points,alpha)
        return x,t
    ...
    def TEST(self,points):
        ...
        for i in range(N):
            ...
            for j in tqdm(range(TRIES)):
                t,x,H = self.make_sample(samples_list[i],points,torch.rand(1,)*2*torch.pi)
...
if __name__ == "__main__":
    ...
    #creator.TEST(128)
    dataset,t = creator.dataset_onekind("fig8",20,256)
    H = creator.hamiltonian(dataset)
    print(H)
