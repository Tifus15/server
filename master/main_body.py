...
from GRU_model import rollout_GNN_GRU_inverted
...
def train6body(configs):
    ...
    t = torch.linspace(0,5.11,512)[0:TIME_SIZE]
    H = torch.load("H6b1000.pt")[0:100,0:256].transpose(0,1).unsqueeze(-1).unsqueeze(-1)
    x = torch.load("x6b1000.pt")[0:100,0:256,:,:].transpose(0,1)
    dx = torch.load("dx6b1000.pt")[0:100,0:256,:,:].transpose(0,1)
    temp = H
    for i in range(5):
    ...
    model = rollout_GNN_GRU_inverted(graph,4,MODEL_SIZE,MODEL_SIZE,ACT_FUNC,type=MODEL)
    ...
    visualize_loss("loss of GNN N6_single",container)
    torch.save(model.state_dict(),"server_6body_single.pth")
...
def train7body(configs):
    ...
    t = torch.linspace(0,5.11,512)[0:TIME_SIZE]
    H = torch.load("H7b1000.pt")[0:100,0:256].transpose(0,1).unsqueeze(-1).unsqueeze(-1)
    x = torch.load("x7b1000.pt")[0:100,0:256,:,:].transpose(0,1)
    dx = torch.load("dx7b1000.pt")[0:100,0:256,:,:].transpose(0,1)
    temp = H
    for i in range(6):
    ...
    #model = rollout_GNN_GRU(graph,4,MODEL_SIZE,MODEL_SIZE,ACT_FUNC,type=MODEL)
    model = rollout_GNN_GRU_inverted(graph,4,MODEL_SIZE,MODEL_SIZE,ACT_FUNC,type=MODEL)
    model = load_model(model,"server_6body_100.pth")
    ...
    visualize_loss("loss of GNN N7_single",container)
    torch.save(model.state_dict(), "server_7body_single.pth")
    print(ROOT_PATH)
def full(configs):
    print(ROOT_PATH)
    print("6body")    
